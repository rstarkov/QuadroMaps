...
internal class DistanceMatrix
{
    ...
    public void Spread(float step)
    {
        ...
        foreach (var tpt in Points.OrderBy(p => Rnd.NextDouble()))
        {
            ...
            {
                var pt = new Pt { Lon = tpt.Point.Lon + (float)(step * Math.Cos(angle)), Lat = tpt.Point.Lat + (float)(step * Math.Sin(angle)) };
                var dist = Points.Select(p => p.Point).Concat(points).Select(p => p.AngDist(pt)).MinOrDefault(double.MaxValue);
                if (dist < step * 0.9)
                    return;
                points.Add(pt);
            }
    ...
    public List<TriangleD> SpreadBoundaryDelaunay(double time)
    {
        ...
        var newPoints = edgeTriangles.Select(t => t.Centroid).Select(c => new Pt { Lon = (float)c.X, Lat = (float)c.Y }).ToList();
        ...
        {
            newPoints = newPoints
                .OrderByDescending(p => newPoints.Where(np => p != np).Concat(Points.Select(pt => pt.Point)).Min(pt => pt.AngDist(p)))
        ...
        {
            var worstPoint = newPoints.OrderBy(p => newPoints.Where(np => p != np).Concat(Points.Select(pt => pt.Point)).Min(pt => pt.AngDist(p))).First();
    ...
    public void RefineBoundaryBad(double time)
    {
        ...
        var mindists = inside.Select(pi => outside.Select(po => po.Point.AngDist(pi.Point)).Min()).Order().ToList();
        var boundary = mindists[mindists.Count / 2];
        var points = new List<Pt>();
        var pairs = inside.SelectMany(pi => outside.Where(po => po.Point.AngDist(pi.Point) < boundary * 1.5).Select(po => (pi, po)));
        foreach (var (pi, po) in pairs)
        {
            var pt = new Pt { Lat = (pi.Point.Lat + po.Point.Lat) / 2, Lon = (pi.Point.Lon + po.Point.Lon) / 2 };
            if (points.Select(p => pt.AngDist(p)).MinOrDefault(double.MaxValue) < boundary / 2)
                continue; // too close to another new point to be queried
            if (Points.Select(p => pt.AngDist(p.Point)).MinOrDefault(double.MaxValue) < 0.8 * boundary / 2)
        ...
        void subedge(List<Pt> edge)
        {
            foreach (var pe in edge)
            {
                var nearest = edge.Where(p => p != pe).OrderBy(p => p.AngDist(pe)).Take(3);
                foreach (var pn in nearest)
                {
                    var pt = new Pt { Lat = (pe.Lat + pn.Lat) / 2, Lon = (pe.Lon + pn.Lon) / 2 };
                    if (points.Select(p => pt.AngDist(p)).MinOrDefault(double.MaxValue) < 0.8 * boundary / 2)
                        continue; // too close to another new point to be queried
                    if (Points.Select(p => pt.AngDist(p.Point)).MinOrDefault(double.MaxValue) < 0.8 * boundary / 2)
                        continue; // too close to another already existing point
                    points.Add(pt);
